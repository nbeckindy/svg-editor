...
export class OutlineToPathCommand implements EditorCommand {
  ...
  private readonly originalMarkup: LiveTreeMarkup;
  private readonly insertionIndex: number;

  constructor(
    private readonly svc: EditorShapeLifecycleSvgPort,
    private readonly shapeId: string,
    // Markup generated by buildOutlineToPathMarkup — editor-created, safe to brand as LiveTreeMarkup.
    private readonly pathMarkup: LiveTreeMarkup,
    ...
    const shape = svgInstance?.findOne(`#${this.shapeId}`) as SvgJsElement | undefined;
    if (!shape?.node) {
      this.originalMarkup = '' as LiveTreeMarkup;
      this.insertionIndex = insertionIndex;
      return;
    }
    // outerHTML comes from the Live tree, which was sanitized at ingest (ADR 0002).
    this.originalMarkup = (shape.node as Element).outerHTML as LiveTreeMarkup;
    this.insertionIndex = insertionIndex;
  }
...
export class BooleanPathCommand implements EditorCommand {
  ...
  private readonly operandSerializedMarkup = new Map<string, LiveTreeMarkup>();
  ...
  constructor(
    ...
    // Markup generated by buildBooleanResultPathMarkup — editor-created, safe to brand as LiveTreeMarkup.
    private readonly resultMarkup: LiveTreeMarkup,
  ...
  private captureOperandState(): void {
    ...
    for (const id of this.operandIds) {
      ...
      // outerHTML comes from the Live tree, which was sanitized at ingest (ADR 0002).
      this.operandSerializedMarkup.set(id, (shape.node as Element).outerHTML as LiveTreeMarkup);
      if (contentNode) {
