...
- Fill paint and **fill/stroke opacity** live under Colors; stroke paint/styling under Stroke; geometry/typography under Properties; artboard under **Document settings panel**. Element-level SVG `opacity` is preserved on import and for layers, but Colors chrome edits `fill-opacity` / `stroke-opacity` (not whole-element `opacity`).
