...
- Multiline **`<text>`** uses newline ↔ canonical per-line **`<tspan>`** (`dy="1.2em"`) on content commit only; imported tspan markup is left intact until the user commits a changed inline-edit draft. Per-span styling and nested tspans are out of scope.
...
- **`DockPanelRegistryService`** + **`registerDefaultDockPanels()`** register the eight stack sections; ids are free-form strings (`document`, `properties`, `text`, `colors`, `stroke`, `alignDistribute`, `layers`, `pathOps`).
