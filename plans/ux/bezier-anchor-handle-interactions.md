...
| **Pen tool (creation)** | **Click** adds a corner (**L**). **Click + drag** (past the same screen threshold as marquee gestures) adds a curve: default **C** keeps the **previous anchor’s outgoing** on the **chord-third** toward the new point; the **new anchor’s incoming** handle lies **opposite the drag** at length **chord/3**, matching the outgoing length (symmetric handle magnitudes at the new knot without moving the start side). **Alt** switches to **end-handle-only** (`P2` follows pointer, `P1` still chord-thirds). **Ctrl/Cmd + alternate curve types (`Q` / `S` / `T`)** are **off** until rediscovered with clear in-app affordance (`svg-editor-h76`). A future preference could expose other placement styles (e.g. SVGator-style). Pen can **insert** an anchor on an existing path when clicking near a segment (separate from node-edit mode). |
