...
| **Pen tool (creation)** | **Click** adds a corner (**L**). **Click + drag** (past the same screen threshold as marquee gestures) adds a curve: default **C** adjusts **only the new vertex’s incoming handle** with the pointer (the prior anchor’s outgoing handle stays on chord thirds); **Alt** mirrors both handles (`P1 = P0 + P3 − P2`). Pen can **insert** an anchor on an existing path when clicking near a segment (separate from node-edit mode). |
