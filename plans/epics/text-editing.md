...
| TE-4 | Create new text element tool | `svg-editor-nkz.4` | `story` | Text tool in tool strip; click on canvas places new `<text>` and enters inline edit. AC: (1) `EditorTool` extended with `text` entry (reuse SC-1 pattern); (2) click on canvas inserts `<text>` via `addShape` API (from SC-2a) at click point in SVG user space; (3) default text content (e.g. "Text"); (4) auto-enters inline edit mode after placement; (5) uses `AddShapeCommand` for undo; (6) text tool remains active after placement and while typing (click outside commits then can place another). | TE-1a, SC-1, SC-2a, SC-5 | 180 |
