...
| APE-7 | Pen tool snapping (grid and shape guides) | `svg-editor-4nz.4` | `feature` | Pen anchor placement snaps to grid when Grid snap is enabled and honors smart guides when Shape snap is enabled, with independent toggle support for grid-only, shape-only, both, or neither. Shift/pen modifiers preserve existing precedence, undo/redo remains correct, and tests cover snapping behavior and toggle combinations for pen interactions. | `svg-editor-gh9`, `svg-editor-l6c.9` | 180 |
